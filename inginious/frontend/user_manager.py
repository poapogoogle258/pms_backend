...
class UserManager:
    ...
    def user_saw_task(self, username, courseid, taskid ):
        ...
        self._database.user_tasks.update({"username": username, "courseid": courseid, "taskid": taskid },
                                         ...
                                         upsert=True)
    #PMS fucntion                                     
    def user_saw_problem(self, username, courseid, taskid, problemid ):
        self._database.user_tasks.update({"username": username, "courseid": courseid, "taskid": taskid ,"problemid":problemid},
                                         {"$setOnInsert": {"username": username, "courseid": courseid, "taskid": taskid,"problemid":problemid,
                                                           "tried": 0, "succeeded": False, "grade": 0.0,
                                                           "submissionid": None, "state": ""}},
                                         upsert=True)
    ...
    #PMS function
    def update_user_stats_with_quality(self, username, task, submission, result_str, grade , memory, runtime, status_work, state, newsub):
        """ Update stats with a new submission """
        self.user_saw_problem(username, submission["courseid"], submission["taskid"],submission['problemid'])

        if newsub:
            old_submission = self._database.user_tasks.find_one_and_update(
                {"username": username, "courseid": submission["courseid"], "taskid": submission["taskid"],"problemid":submission['problemid'] },
                {"$inc": {"tried": 1, "tokens.amount": 1}})
            ...
            if set_default:
                self._database.user_tasks.find_one_and_update(
                    {"username": username, "courseid": submission["courseid"], "taskid": submission["taskid"],"problemid":submission['problemid']},
                    {"$set": {"succeeded": result_str == "success", "grade": grade,"memory":memory,"runtime":runtime, "state": state,'status_work':status_work,"submitted_on":submission['submitted_on'],
                              "submissionid": submission['_id']}})
        else:
            old_submission = self._database.user_tasks.find_one(
                {"username": username, "courseid": submission["courseid"], "taskid": submission["taskid"],"problemid":submission['problmeid']})
            old_seccessed = old_submission['succeeded'] 


            def_sub = []
            if task.get_evaluate() == 'best':  # if best, update cache consequently (with best submission)
                def_sub = list(self._database.submissions.find(
                    {"username": username, "courseid": submission["courseid"], "taskid": submission["taskid"],"problemid":submission['problmeid'],"status": "done"})
                    .sort([("runtime", pymongo.ASCENDING),("memory", pymongo.ASCENDING),("submitted_on", pymongo.DESCENDING)]).limit(1))
            ...
            if len(def_sub) > 0:
                self._database.user_tasks.find_one_and_update(
                    {"username": username, "courseid": submission["courseid"], "taskid": submission["taskid"],"problemid":submission['problmeid']},
                    {"$set": {
                        "succeeded": True if old_seccessed else submissio["result"] == "success",
                        "grade": def_sub[0]["grade"],
                        "state": def_sub[0]["state"],
    ...
    #PMS 
    def get_problem_caches(self, usernames, courseid, taskid, problemid):
        """
        :param usernames: List of username for which we want info. If usernames is None, data from all users will be returned.
        :param courseid: the course id
        :param taskid: the task id
        ...
        """
        match = {"courseid": courseid, "taskid": taskid, "problemid":problemid}
        if usernames is not None:
            match["username"] = {"$in": usernames}
