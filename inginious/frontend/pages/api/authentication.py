...
class APIAuthentication(APIPage):
    ...
    def API_GET(self):  # pylint: disable=arguments-differ
        ...
        if self.user_manager.session_logged_in():
            return 200, {"authenticated": True, "username": self.user_manager.session_username(),'realname':self.user_manager.get_user_realname(self.user_manager.session_username())}
        else:
            return 200, {"authenticated": False}

    def API_POST(self):  # pylint: disable=arguments-differ
        """
            Authenticates the remote client. Takes as input:realname
        ...
        try:
            if self.user_manager.auth_user(user_input["login"].strip(), user_input["password"]) is not None:
                    return 200, {"status": "success", "username": self.user_manager.session_username(),'realname':self.user_manager.get_user_realname(self.user_manager.session_username())}
        except:
            pass
