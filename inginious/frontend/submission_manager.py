...
class WebAppSubmissionManager:
    ...
    def _job_done_callback(self, submissionid, task, result, grade, problems, tests, custom, state, archive, stdout,
        ...
        data = {
            ...
            "custom": custom,
            "memory":custom.get('memory',None), #PMS15-10
            "runtime":custom.get('runtime',None), #PMS15-10
            "status_work":custom.get('status_work',None), #PMS15-10
            "state": state,
            "stdout": stdout,
            "stderr": stderr
        }
        ...
        for username in submission["username"]:
            ...
            self._user_manager.update_user_stats_with_quality(username, task, submission, result[0], grade ,custom.get('memory',None),custom.get('runtime',None),custom.get('status_work',None), state, newsub)
    ...
    def replay_job(self, task, submission, copy=False, debug=False):
        ...
        if not copy:
            ...
            my_user_task = self._database.user_tasks.find_one(
                {"courseid": task.get_course_id(), "taskid": task.get_id(), "username": username,"problemid":submission['problemid']},
                {"tried": 1, "_id": 0})                
    ...
    def add_job(self, task, inputdata, debug=False):
        ...
        waiting_submission = self._database.submissions.find_one({
            ...
            "taskid": task.get_id(),
            "problemid" : inputdata['problemid'], #PMS15-10
            "username": username,
            "status": "waiting"})
        ...
        my_user_task = self._database.user_tasks.find_one({"courseid": task.get_course_id(), "taskid": task.get_id(),"username": username,'problemid':inputdata['problemid']}, {"tried": 1, "_id": 0})
        tried_count = my_user_task["tried"]
        inputdata["@attempts"] = str(tried_count + 1)
        # Retrieve input random
        states = self._database.user_tasks.find_one(
            {"courseid": task.get_course_id(), "taskid": task.get_id(), "username": username,"problemid":inputdata['problemid']},
            {"random": 1, "state": 1})
